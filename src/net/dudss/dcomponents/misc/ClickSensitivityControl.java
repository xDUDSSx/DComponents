...
public class ClickSensitivityControl implements AWTEventListener {
	...
	private static boolean installedAlready = false;
	...
	public synchronized static void install(int clickPixelTolerance) {
		if (!installedAlready) {
			ClickSensitivityControl csc = new ClickSensitivityControl(clickPixelTolerance);
			Toolkit.getDefaultToolkit().addAWTEventListener(csc, AWTEvent.MOUSE_EVENT_MASK + AWTEvent.MOUSE_MOTION_EVENT_MASK);
